...
export default function AttachmentSection({ ticketId, attachments, onChange }: Props) {
  ...
  async function handleAdd(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) { setUploadError(`${file.name}: file type not allowed`); return; }
    if (file.size > MAX_SIZE) { setUploadError(`${file.name}: exceeds 5MB`); return; }
    if (activeCount >= 5) { setUploadError("This ticket already has the maximum of 5 active attachments."); return; }
  ...
  async function handleConfirmRemove(id: number) {
    ...
    try {
      const updated = await removeAttachment(id, reason.trim());
      onChange(attachments.map((a) => (a.id === id ? { ...a, isRemoved: true, removedReason: updated.removedReason } : a)));
      setRemovingId(null);
      setReason("");
    } catch {
    ...
    <div className="card p-3 mt-3">
          ...
          <li key={a.id} className="list-group-item d-flex justify-content-between align-items-center">
            ...
            {!a.isRemoved && (
              <div className="d-flex gap-2">
                ...
                <button className="btn btn-sm btn-outline-danger" onClick={() => setRemovingId(a.id)} aria-label={`Remove ${a.originalFilename}`}>Remove</button>
              </div>
            )}
          </li>
        ))}
      ...
      {removingId !== null && (
        <div className="mb-3 border rounded p-2">
          ...
          <input className="form-control mb-2" value={reason} onChange={(e) => setReason(e.target.value)} />
          <button className="btn btn-sm btn-danger me-2" disabled={reason.trim().length < 3} onClick={() => handleConfirmRemove(removingId)}>Confirm Removal</button>
          <button className="btn btn-sm btn-outline-secondary" onClick={() => { setRemovingId(null); setReason(""); }}>Cancel</button>
        </div>
      )}
