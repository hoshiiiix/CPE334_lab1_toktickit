...
export default function MyTickets() {
  ...
  function load() {
    setState("loading");
    fetchTickets({ search: search || undefined, requestedPriority: requestedPriority || undefined, page })
      .then((res) => {
        setTickets(res.data);
        setPagination(res.pagination);
        const hasFilters = Boolean(search || requestedPriority);
    ...
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="d-flex gap-2">
          ...
          <Link to="/tickets/new" className="btn btn-success btn-sm">+ Create Ticket</Link>
        </div>
      </div>
      ...
      <form className="row g-2 mb-3" onSubmit={handleSearchSubmit}>
        ...
        <div className="col-md-3">
          <select className="form-select" value={requestedPriority} onChange={(e) => { setRequestedPriority(e.target.value); setPage(1); setTimeout(load, 0); }}>
      ...
      {state === "error" && <p className="text-danger">Unable to load your tickets right now.</p>}
        ...
        <>
          <div className="table-responsive d-none d-md-block">
            <table className="table bg-white">
              <thead>
                <tr><th>Ticket No.</th><th>Created</th><th>Summary</th><th>Requested Priority</th><th>Status</th></tr>
              </thead>
              <tbody>
                {tickets.map((t) => (
                  <tr key={t.id}>
          ...
          <div className="d-md-none">
            {tickets.map((t) => (
              <Link to={`/tickets/${t.id}`} key={t.id} className="card p-2 mb-2 text-decoration-none text-dark">
